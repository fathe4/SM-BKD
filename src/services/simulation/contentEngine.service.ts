...
const STOP_WORDS = new Set([
    "the", "and", "for", "you", "not", "but", "are", "was", "with", "from",
    "how", "why", "that", "this", "these", "those", "have", "has", "had",
    "been", "were", "what", "when", "where", "who", "which", "will", "would",
    "should", "can", "could", "about", "their", "them", "then", "there",
    "than", "some", "only", "other", "same", "such", "very", "also", "just",
    "your", "yours", "its", "into", "here", "more", "most", "each"
]);
...
function isTitleSimilar(titleA: string, titleB: string): boolean {
    ...
    const wordsA = cleanA.split(/\s+/).filter(w => w.length >= 3 && !STOP_WORDS.has(w));
    const wordsB = cleanB.split(/\s+/).filter(w => w.length >= 3 && !STOP_WORDS.has(w));
    ...
    // If they share 3 or more significant non-stop words, OR have high Jaccard-like similarity, they are too similar
...
export class ContentEngineService {
    ...
    static async generateAndPublishPost(personaId: string, postingProfileName: string, forcePostType?: string): Promise<void> {
        try {
            ...
            // Fetch ALL titles of candidates that have already been posted globally by any AI persona
            const { data: allUsedPosts } = await supabaseAdmin!
                .from("posts")
                .select("source")
                .not("source", "is", null);

            const publishedSourcesSet = new Set(
                (allUsedPosts || []).map(p => p.source).filter(Boolean)
            );

            // Fetch image URLs of all posts that have ever been published globally
            const { data: allUsedMedia } = await supabaseAdmin!
                .from("post_media")
                .select("media_url")
                .not("media_url", "is", null);

            const publishedMediaUrlsSet = new Set(
                (allUsedMedia || []).map(m => m.media_url).filter(Boolean)
            );
            ...
            const getUnusedCandidates = (cands: any[]) => {
                return (cands || []).filter(cand => {
                    ...
                    // Skip candidates that have ever been published globally
                    if (publishedSourcesSet.has(cand.title)) return false;
            ...
            if (matchedCandidates.length > 0) {
                ...
                for (const cand of matchedCandidates) {
                    ...
                    if (publishedSourcesSet.has(cand.title)) {
                    ...
                    if (cand.imageurl && publishedMediaUrlsSet.has(cand.imageurl)) {
