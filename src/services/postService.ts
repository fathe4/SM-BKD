...
import { AppError, ExpectedError } from "../middlewares/errorHandler";
...
export class PostService {
    ...
    static createPost = asyncHandler(
        async (postData: PostCreate): Promise<Post> => {
            ...
            if (error) {
                if (error.code === "23505" || error.message?.includes("posts_source_unique_idx")) {
                    throw new ExpectedError("Duplicate post: source already exists in database", 409);
                }
     ...
     */
    public static invalidateRelevantFeeds = async (
        userId: string,
        location?: any
    ): Promise<void> => {
        try {
